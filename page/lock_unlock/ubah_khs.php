...
<form method="POST" enctype="multipart/form-data">
	<div class="box box-primary">
	   <div class="box-header with-border">
	     	<h3 class="box-title">Ubah Akses KHS Mahasiswa</h3>
	   </div>
	   <div class="box-body">
	    ...
	    <div class="form-group">
			  <label>Akses KHS</label>
			  <select class="form-control" required name="khs">
			     <option value="0" <?php if( $tampil['khs']=='0'){echo "selected"; } ?>>Lock</option>
			     <option value="1" <?php if( $tampil['khs']=='1'){echo "selected"; } ?>>Unlock</option>
			  </select>
			</div>
...
<?php 
	if (isset($_POST['simpan'])) {
		...
		if(!empty($lokasi)) {
			...
			$koneksi->query("UPDATE  tb_mahasiswa SET  khs = '$khs' WHERE nim = '$nim'");
				?>
			       <script>
					    setTimeout(function() {
					        swal({
