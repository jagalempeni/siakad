	...
	class QRcode {
		 ...
		 */
		 protected function eatNum() {
			...
			$this->items = $this->appendNewMasukanItem($this->items, QR_MODE_NM, $run, str_split($this->dataStr));
			return $run;
		}
		 ...
		 */
		 protected function eatAn() {
			...
			$this->items = $this->appendNewMasukanItem($this->items, QR_MODE_AN, $run, str_split($this->dataStr));
			return $run;
		}
		 ...
		 */
		 protected function eatKanji() {
			...
			$this->items = $this->appendNewMasukanItem($this->items, QR_MODE_KJ, $p, str_split($this->dataStr));
			return $run;
		}
		 ...
		 */
		 protected function eat8() {
			...
			$run = $p;
			$this->items = $this->appendNewMasukanItem($this->items, QR_MODE_8B, $run, str_split($this->dataStr));
			return $run;
		}
		 ...
		 */
		 protected function newMasukanItem($mode, $size, $data, $bstream=null) {
		 ...
		 */
		 protected function encodeBitStream($inputitem, $version) {
			...
			if ($inputitem['size'] > $words) {
				$st1 = $this->newMasukanItem($inputitem['mode'], $words, $inputitem['data']);
				$st2 = $this->newMasukanItem($inputitem['mode'], $inputitem['size'] - $words, array_slice($inputitem['data'], $words));
		 ...
		 *
		 */
		protected function appendNewMasukanItem($items, $mode, $size, $data) {
			$items[] = $this->newMasukanItem($mode, $size, $data);
			return $items;
		}
		 ...
		 */
		 protected function insertStructuredAppendHeader($items, $size, $index, $parity) {
			...
			$entry = $this->newMasukanItem(QR_MODE_ST, 3, buf);
